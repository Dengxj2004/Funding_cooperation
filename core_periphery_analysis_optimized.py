...
def read_single_file(path: str) -> pd.DataFrame:
    ...
    if miss:
        raise ValueError(f"File {path} missing required columns: {miss}")
...
def plot_dashboard(metrics_df: pd.DataFrame, time_col: str, out_path: str):
    ...
    axes[0, 0].plot(x, metrics_df["hhi_degree"], marker="o", linewidth=2.8, color=BLUE_GRADIENT[2], label="HHI (Degree)")
    axes[0, 0].plot(x, metrics_df["hhi_strength"], marker="s", linewidth=2.2, color=BLUE_GRADIENT[1], label="HHI (Strength)")
    _style_axis(axes[0, 0], "Concentration (HHI)", "Time", "Index")
    axes[0, 0].legend(frameon=False)
    format_time_ticks(axes[0, 0], x, max_ticks=10)

    axes[0, 1].plot(x, metrics_df["gini_degree"], marker="o", linewidth=2.8, color=GREEN_GRADIENT[2], label="Gini (Degree)")
    axes[0, 1].plot(x, metrics_df["gini_strength"], marker="s", linewidth=2.2, color=GREEN_GRADIENT[1], label="Gini (Strength)")
    axes[0, 1].plot(x, metrics_df["cr5_degree"], marker="^", linewidth=2.0, color=BLUE_GRADIENT[0], label="CR5 (Degree)")
    _style_axis(axes[0, 1], "Inequality and Head Concentration", "Time", "Ratio")
    ...
    axes[1, 0].plot(x, metrics_df["lcc_ratio"], marker="D", linewidth=2.0, color=GREEN_GRADIENT[2], label="LCC Ratio")
    _style_axis(axes[1, 0], "Core Ratio and Connectivity", "Time", "Ratio")
    axes[1, 0].legend(frameon=False)
    format_time_ticks(axes[1, 0], x, max_ticks=10)

    axes[1, 1].plot(x, metrics_df["nodes"], marker="o", linewidth=2.8, color=BLUE_GRADIENT[2], label="Nodes")
    axes[1, 1].plot(x, metrics_df["edges"], marker="s", linewidth=2.8, color=GREEN_GRADIENT[2], label="Edges")
    ax2 = axes[1, 1].twinx()
    ax2.plot(x, metrics_df["density"], marker="^", linewidth=2.0, color=BLUE_GRADIENT[0], linestyle="--", label="Density")
    axes[1, 1].set_title("Network Size and Density", fontsize=16, pad=10)
    axes[1, 1].set_xlabel("Time", fontsize=12)
    axes[1, 1].set_ylabel("Scale", fontsize=12)
    ...
    fig.suptitle("NSFC-funded Institutional Collaboration Network: Core-Periphery Evolution", fontsize=20, y=1.02)
...
def plot_metric_heatmap(metrics_df: pd.DataFrame, time_col: str, out_path: str):
    ...
    ax.set_title("Standardized Multi-metric Heatmap (by Time)", fontsize=16)
    ax.set_xlabel("Time")
...
def plot_bubble_tradeoff(metrics_df: pd.DataFrame, time_col: str, out_path: str):
    ...
    _style_axis(ax, "Density-Concentration Tradeoff (Bubble=Node Size, Color=Core Ratio)", "Density", "HHI (Strength)")
...
def plot_edge_weight_violin(edge_weight_df: pd.DataFrame, time_col: str, out_path: str):
    ...
    _style_axis(ax, "Evolution of Collaboration Strength (Edge Weight)", "Time", "Edge Weight (Co-occurrence Count)")
...
def plot_top_institutions_bar(top_inst_df: pd.DataFrame, time_col: str, out_path: str):
    ...
    _style_axis(ax, f"{latest} Top 15 Core Institutions (by Strength)", "Weighted Degree (Strength)", "Institution")
    fig.tight_layout()
    fig.savefig(out_path, dpi=350)
    plt.close(fig)
...
def plot_top_institution_stream(top_inst_df: pd.DataFrame, time_col: str, out_path: str):
    ...
    _style_axis(ax, "Stacked Evolution of Top-Institution Collaboration Strength (Top 8)", "Time", "Collaboration Strength")
...
def quality_check(df_before_filter: pd.DataFrame, df_after_filter: pd.DataFrame, metrics_df: pd.DataFrame) -> str:
    ...
    msg = [
        "=== Data Quality Check ===",
        f"Missing ratio grantno/address/year: {null_ratio.to_dict()}",
        f"Share of samples with <2 institutions (before filter): {bad_inst}",
        f"Number of analyzed periods: {len(metrics_df)}",
        f"Raw sample size: {len(df_before_filter)}, post-filter sample size: {len(df_after_filter)}",
    ]
    if metrics_df[["hhi_degree", "gini_degree", "cr5_degree"]].isna().any().any():
        msg.append("Warning: Some periods are too sparse, leading to NaN concentration metrics (kept intentionally).")
    return "\n".join(msg)
...
def run(file1: str, file2: str, out_dir: str, n_jobs: int, yearly_min_rows: int, font_cn: str, font_en: str):
    ...
    print(qc)
    print(f"Time granularity: {time_col}, periods: {time_values}")
    print(f"Output directory: {out_dir}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Core-periphery evolution analysis for NSFC-funded institutional collaboration (parallel + multi-visual outputs)")
    parser.add_argument("--file1", required=True, help="TSV file before 2022")
    parser.add_argument("--file2", required=True, help="CSV file for 2022-2023")
    parser.add_argument("--out_dir", default="./output_core_periphery")
    parser.add_argument("--n_jobs", type=int, default=0, help="Number of parallel workers; 0 means auto use all CPUs")
    parser.add_argument("--yearly_min_rows", type=int, default=200, help="Minimum sample threshold for yearly analysis")
