...
def load_metric_files(metrics_dir: str):
    ...
    if not os.path.exists(metrics_path):
        raise FileNotFoundError(f"Metric file not found: {metrics_path}")
...
def plot_dashboard(metrics_df: pd.DataFrame, time_col: str, out_dir: str):
    ...
    style_axis(axes[0, 0], "Concentration (HHI)", "Time", "Index")
    ...
    style_axis(axes[0, 1], "Inequality and Head Concentration", "Time", "Ratio")
    ...
    style_axis(axes[1, 0], "Core Ratio and Connectivity", "Time", "Ratio")
...
def plot_bubble(metrics_df: pd.DataFrame, time_col: str, out_dir: str):
    ...
    style_axis(ax, "Density-Concentration Tradeoff (Bubble=Node Size, Color=Core Ratio)", "Density", "HHI (Strength)")
...
def plot_violin(edge_df: pd.DataFrame, time_col: str, out_dir: str):
    ...
    style_axis(ax, "Evolution of Collaboration Strength (Edge Weight)", "Time", "Edge Weight (Co-occurrence Count)")
...
def plot_top_bar(top_df: pd.DataFrame, time_col: str, out_dir: str):
    ...
    style_axis(ax, f"{latest} Top 15 Core Institutions (by Strength)", "Weighted Degree (Strength)", "Institution")
...
def plot_stream(top_df: pd.DataFrame, time_col: str, out_dir: str):
    ...
    style_axis(ax, "Stacked Evolution of Top-Institution Collaboration Strength (Top 8)", "Time", "Collaboration Strength")
...
def main(args):
    ...
    print("Done: Core-periphery figures redrawn from existing metrics.")
    print(f"Metrics directory: {args.metrics_dir}")
    print(f"Output directory: {args.out_dir}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Redraw core-periphery figures from precomputed metrics without re-reading raw data")
